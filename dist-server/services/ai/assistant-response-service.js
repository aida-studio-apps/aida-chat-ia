...
function getFullError(error) {
    return [error?.status, error?.code, error?.error?.message || error?.message]
        .filter(Boolean)
        .join(' | ');
}
export async function generateAssistantResponse(conversationId, userContent) {
    try {
        ...
        const messages = [
            ...
            ...history.map((m) => ({ role: m.author === 'user' ? 'user' : 'assistant', content: m.content })),
        ...
        try {
            ...
            keyFacts = Array.isArray(parsed.keyFacts)
                ? parsed.keyFacts.slice(0, 5).filter((x) => typeof x === 'string')
                : [];
        }
        catch {
